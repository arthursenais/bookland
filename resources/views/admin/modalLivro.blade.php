...
@isset($livro)
    ...
    <div id="modalLivro-{{ $livro->id }}" class="fixed inset-0 z-40 items-center justify-center hidden bg-gray-900/50">
        <div class="flex flex-col bg-white dark:bg-slate-800 rounded p-4 dark:text-white sm:min-w-[500px]">
            ...
            <form method="POST" action="{{ route('admin.updateLivro', $livro->id) }}" enctype="multipart/form-data"
                ...
                <div>
                    ...
                    <input type="file" id="imageupload-{{ $livro->id }}" name="imagem" class="hidden modifyimg">
                </div>
                ...
                <table class="sm:w-full">
                    <tr>
                        <th>titulo:</th>
                        <th class="w-full">
                            <input name="titulo" required type="text" value="{{ $livro->titulo }}"
                                class="w-full text-right border dark:border-slate-700 rounded p-1 truncate dark:bg-slate-800">
                        </th>
                    </tr>
                    <tr>
                        <th>autor:</th>
                        <th class="text-right">
                            <input name="autor" required type="text" value="{{ $livro->autor }}"
                    ...
                    <tr>
                        ...
                        <th class="text-right">
                            <select required name="id_categoria"
                                class="dark:bg-slate-800 w-fit truncate  border dark:border-slate-700 rounded p-1 text-right rounded p-0.5 invalid:border invalid:border-red-600/20 focus:outline-none focus:ring invalid:focus:ring-red-600">
                                @foreach ($categorias as $categoria)
                                    <option value="{{ $categoria->id }}"
                                        {{ $categoria->id == $livro->categoria->id ? 'selected' : '' }}>
                ...
                <div class="flex justify-between w-full">
                    ...
                    <button type="button" onclick="modalDelete({{ $livro->id }})"
...
<div id="modalAddLivro" class="fixed inset-0 z-50 items-center justify-center hidden bg-gray-900/50 backdrop-blur">
    <div class="flex flex-col bg-white dark:bg-slate-800 rounded p-4 dark:text-white sm:min-w-[500px]">
        ...
        @if (count($categorias) > 0)
            <form action="{{ route('admin.storeLivro') }}" method="POST" enctype="multipart/form-data"
                class="flex flex-col items-center justify-center gap-4 text-left align-center">
                @csrf
                <div class="flex justify-start w-full">
                    <table class="sm:w-full">
                        <tr>
                            <th>titulo:</th>
                            <td class="w-full">
                                <input name="titulo" type="text" placeholder="Digite aqui" required
                        ...
                        <tr>
                            <th>autor:</th>
                            <td class="w-full">
                                <input name="autor" type="text" placeholder="Digite aqui" required
                                    class="dark:bg-slate-800 w-full truncate  rounded p-0.5   invalid:border invalid:border-red-600/20 focus:outline-none focus:ring invalid:focus:ring-red-600">
                            </td>
                        </tr>
                        <tr>
                            ...
                            <td class="w-full">
                                ...
                                <select required name="id_categoria"
                                    class="dark:bg-slate-800 w-fit truncate  rounded p-0.5   invalid:border invalid:border-red-600/20 focus:outline-none focus:ring invalid:focus:ring-red-600">
                                    @foreach ($categorias as $categoria)
                        ...
                        <tr>
                            <th>disponiveis:</th>
                            <td class="w-full">
                                <input type="number" name="disponiveis" min="0" max="100" required
                    ...
                    <div class="min-w-[250px] flex flex-col items-center relative">
                        ...
                        <label for="addImagem" id="icon"
                            class="p-2 text-white transition rounded-full cursor-pointer material-icons button bg-emerald-500 hover:bg-emerald-600">image</label>
                        <input type="file" id="addImagem" required name="imagem" class="hidden ">
                        <p id="loadImg">Imagem</p>
                    </div>
                </div>
                <p>Descrição:</p>
        ...
        $('#addImagem').change(function() {
            ...
            if (this.files && this.files[0]) {
                ...
                reader.onload = function(e) {
                    ...
                    $('#icon').addClass('absolute bottom-3');
                    $('#loadImg').html('Mudar imagem');
                }
